...
const MEDIA_COLLECTION = 'x-media-items'
const ACCESS_LOGS_COLLECTION = 'x-access-logs'
const SHARED_STATE_COLLECTION = 'x-shared-state'
const SHARED_PLAYLISTS_DOC = 'playlists'
const SHARED_SPACES_DOC = 'spaces'
const CHAT_THREADS_COLLECTION = 'x-chatThreads'
const CHAT_PROFILES_COLLECTION = 'x-chatProfiles'
const PUSH_TOKENS_COLLECTION = 'x-pushTokens'
const AVATAR_CACHE_PREFIX = 'x-chat-avatar-'
const GUEST_CHAT_ID_KEY = 'x-chat-guest-id'
...
export async function uploadUserAvatar(profileId, file, meta = {}) {
  ...
  const objectRef = storageRef(storage, `x-avatars/${id}.jpg`)
...
export async function uploadChatImage(threadId, file) {
  ...
  const objectRef = storageRef(storage, `x-chat-images/${tid}/${stamp}.jpg`)
  await uploadBytes(objectRef, blob, { contentType: 'image/jpeg' })
...
const ACCESS_LOG_SESSION_KEY = 'xmediabox-access-logged'
...
export async function uploadMediaFile(file, metadata, onProgress) {
  ...
  if (shouldUseObjectStorage(file, metadata)) {
    const path = `x-uploads/${id}/${sanitizeStorageFileName(file.name || 'book.pdf')}`
